<script>
  ...
  const tournament = tournaments.find(obj => {
    return obj.attributes.apiid == tournamentId
  })

  let allSessions = []
  for (const session of tournament.attributes.sessions.data){
    console.log("session "+session.attributes.name)
...
//  let openPartialScores = false
  ...
  const calculateSessionPartialScores = (session) => {
    const data = []
    for (const [index, user] of session.attributes.archers.entries()) {
      data.push({
        username: user.username,
        score: calculatePartialScoreForUser(session.attributes.scores[index]),
      })
    }
...
<div class="tournaments-annotations">
  <SectionHeader>
    <h1 slot="title">(O) {tournament.attributes.name}</h1>
    <Button slot="action">
  ...
  <main>
      ...
      <div class="archer">
        <h2 class="session">{session.attributes.name}</h2>
      
        <div class="total">

        {#each calculateSessionPartialScores(session) as archer, u}
        
          
            <p>{archer.username}{#if archer.username === you.username}{" (You)"}{/if}: {archer.score}</p>
